...
fn derive_struct(input: &syn::DeriveInput) -> Result<proc_macro2::TokenStream, syn::Error> {
    ...
    for gp in input.generics.params.iter() {
        if let GenericParam::Type(TypeParam { ident, .. }) = gp {
            ...
            let phantom_ident = Ident::new(&format!("phantom_{}", ident), Span::call_site());
            phantom_decls.push(quote! {
                    pub #phantom_ident: std::marker::PhantomData<*const #ident>
            });
            phantom_inits.push(quote! {
    ...
    let defs = fields.iter().filter(|f| !f.attrs.ignore).map(|f| {
        ...
        quote! {
            ...
            pub struct #field_name#lens_ty_generics{
                #(#phantom_decls),*
            }
        }
    });
    ...
    let associated_items = fields.iter().filter(|f| !f.attrs.ignore).map(|f| {
        ...
        quote! {
            ...
            pub const #lens_field_name: #twizzled_name::#field_name#lens_ty_generics = #twizzled_name::#field_name{
                         #(#phantom_inits),*
            };
        }
    });
